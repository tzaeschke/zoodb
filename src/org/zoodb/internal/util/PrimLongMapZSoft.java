...
public class PrimLongMapZSoft<T> implements PrimLongMap<T> {

	public static class Entry<T> extends SoftReference<T> implements PrimLongEntry<T> {
		final long key;
		Entry<T> next = null;
	...
	public PrimLongMapZSoft() {
		this(60);
	}
	
	@SuppressWarnings("unchecked")
	public PrimLongMapZSoft(int capacity) {
		capacityPower = 1;
	...
	class EntryIterator implements Iterator<PrimLongEntry<T>> {
		...
		private void findNext() {
			...
			while (next != null && (nextT = next.getValue()) == null) {
				...
				PrimLongMapZSoft.this.remove(next.key);
				next = newSlotDummy;
				currentModCount = modCount;
				findNextUnsafe();
			}
...
//		}
		...
		public void remove() {
			...
			PrimLongMapZSoft.this.remove(prevEntry.key);
			currentModCount = modCount;
		}
	}
	...
	private abstract class ResultSet<R> implements Set<R> {
		@Override
		public int size() {
			return PrimLongMapZSoft.this.size();
		}
