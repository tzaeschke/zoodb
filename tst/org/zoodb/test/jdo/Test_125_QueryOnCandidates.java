...
public class Test_125_QueryOnCandidates {

	@BeforeClass
	public static void setUp() {
        TestTools.removeDb();
		TestTools.createDb();
		TestTools.defineSchema(TestClass.class, TestClassSmall.class);
	...
	@Test
	public void testQueryOnCollectionWithNonComittedNoMP() {
		testQueryOnNonComitted(false);
	}
	
	@Test
	public void testQueryOnCollectionWithNonComittedWithMP() {
	...
	private void testQueryOnNonComitted(boolean makePersistent) {
        ...
        Extent<TestClass> x1 = pm.getExtent(TestClass.class);
        for (TestClass tc: x1) {
        	list.add(tc);
        }
        ...
        Query q = pm.newQuery(TestClass.class, list);
        ...
        ArrayList<TestClass> test = new ArrayList<>();
        test.add(tc1);
        test.add(tc2);
        ...
        //test with filter
        Query q2 = pm.newQuery(TestClass.class, list, "_int >= 88 && _int <= 123");
        checkQuery(q2, 3, tc1, tc2);
        checkRemoveFail(q2, 3);
        
        TestTools.closePM(pm);
	}

	@Test
	public void testQueryOnExtentWithNonComittedNoMP() {
		testQueryOnNonComittedWithExtent(false);
	}

	@Test
	public void testQueryOnExtentWithNonComittedMP() {
	...
	private void testQueryOnNonComittedWithExtent(boolean makePersistent) {
        ...
        Extent<TestClassSmall> x1 = pm.getExtent(TestClassSmall.class);
