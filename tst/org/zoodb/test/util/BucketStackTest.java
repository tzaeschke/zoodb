 ...
 */
public final class BucketStackTest {
    ...
    private BucketStack<String> _bucket;
    ...
    public void before() {
        ...
        _bucket = new BucketStack<String>();
        _bucket.push(ELEMENT1);
        _bucket.push(ELEMENT2);
        _bucket.push(ELEMENT3);
    }
...
//    }
    ...
    public void testSetMany() {
    	...
    	BucketStack<Integer> bucket = new BucketStack<Integer>(10);
    	for (int i = 0; i < MAX; i++) {
    		bucket.push(i);
    		assertEquals(i+1, bucket.size());
    		assertEquals(i, (int)bucket.peek());
    	}
    ...
    public void testRemoveMany() {
    	final int MAX = 10000;
    	BucketStack<Integer> bucket = new BucketStack<Integer>();
    	...
    	for (int i = MAX-1; i >=0; i--) {
    		assertEquals(i, (int)bucket.peek());
    		assertEquals(i, (int)bucket.pop());
    	}
    	
        assertEquals("Check the number of values", 0, bucket.size());
        assertTrue("Check the number of values", bucket.isEmpty());
    }
