...
//		System.out.println("l1=" + len1/ps + " l2=" + f.length()/ps);
	...
	public void testObjectsReusePagesDeletedSchema() {
		...
		for (int j = 0; j < MAX_ITER; j++) {
			...
			ZooSchema.locateClass(pm, TestClass.class).locateField("_int").createIndex(false);
			ZooSchema.locateClass(pm, TestClass.class).locateField("_long").createIndex(true);
			for (int i = 0; i < MAX; i++) {
				TestClass tc = new TestClass();
				tc.setLong(l++);
				pm.makePersistent(tc);
